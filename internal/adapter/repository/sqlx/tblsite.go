...
type TblSiteRepository struct {
	DB *repository.Sqlx `inject:"database"`
}

func (t *TblSiteRepository) Fetch(ctx context.Context, name string, param *pagination.PaginationParam) (*pagination.PaginationResponse, error) {
	var totalRecords int

	search := "%" + name + "%"

	countQuery := "SELECT COUNT(*) FROM tblsite WHERE SiteCode LIKE ? OR SiteName LIKE ?"
	...
	var items []*tblsite.Read
	query := `SELECT SiteCode,
				SiteName,
				Address,
				Active,
				Remark,
	...
	if err := t.DB.SelectContext(ctx, &items, query, search, search, param.PageSize, offset); err != nil {
		...
		return nil, fmt.Errorf("error Fetch Site: %w", err)
	}
...
func (t *TblSiteRepository) Create(ctx context.Context, data *tblsite.Create) (*tblsite.Create, error) {
	query := `INSERT INTO tblsite
				(
					SiteCode,
					SiteName,
					Address,
	...
	if err != nil {
		...
		return nil, fmt.Errorf("error Create Site: %w", err)
	}

	return data, nil
}

func (t *TblSiteRepository) Update(ctx context.Context, data *tblsite.Update) (*tblsite.Update, error) {
	query := `UPDATE tblsite SET
			SiteName = ?,
			Address = ?,
			Active = ?,
			Remark = ?
	...
	_, err = tx.ExecContext(ctx, query, data.LastUpdateBy, data.SiteCode, "Site", data.LastUpdateDate)
